...
public class MirrorPlane : MonoBehaviour {
    ...
    private Matrix4x4 p = new Matrix4x4();        //矩阵
	...
	void Start () {
        mirrorCamera = GetComponent<Camera>();//mirrorCamera 的位置在
    }
	
	// Update is called once per frame
	void Update () {
        if (null == mirrorPlane || null == mirrorCamera) return;
        pa = mirrorPlane.transform.TransformPoint(new Vector3(-5.0f, 0.0f, -5.0f)); //世界坐标系的 镜子的左下角 
        //把plane中的（-5，0，-5）转到世界坐标系 
        ...
        //x，y，z轴
        vr.Normalize();  //单位化向量（置向量的模为1
        vu.Normalize();
        vn = -Vector3.Cross(vr, vu); //两个向量的叉乘，最后再取负，因为Unity是使用左手坐标系
        vn.Normalize();
        ...
        l = Vector3.Dot(vr, va) * n / d;  //向量点乘
        r = Vector3.Dot(vr, vb) * n / d;
        b = Vector3.Dot(vu, va) * n / d;
        t = Vector3.Dot(vu, vc) * n / d;

        //投影矩阵 
        p[0, 0] = 2.0f * n / (r - l); p[0, 1] = 0.0f;                   p[0, 2] = (r + l) / (r - l);     p[0, 3] = 0.0f;
        p[1, 0] = 0.0f;                  p[1, 1] = 2.0f * n / (t - b); p[1, 2] = (t + b) / (t - b);   p[1, 3] = 0.0f;
        p[2, 0] = 0.0f;                  p[2, 1] = 0.0f;                   p[2, 2] = (f + n) / (n - f);   p[2, 3] = 2.0f * f * n / (n - f);
        p[3, 0] = 0.0f;                  p[3, 1] = 0.0f;                   p[3, 2] = -1.0f;                  p[3, 3] = 0.0f;

        //旋转矩阵
        rm[0, 0] = vr.x; rm[0, 1] = vr.y; rm[0, 2] = vr.z; rm[0, 3] = 0.0f;
        rm[1, 0] = vu.x; rm[1, 1] = vu.y; rm[1, 2] = vu.z; rm[1, 3] = 0.0f;
        rm[2, 0] = vn.x; rm[2, 1] = vn.y; rm[2, 2] = vn.z; rm[2, 3] = 0.0f;
        rm[3, 0] = 0.0f; rm[3, 1] = 0.0f; rm[3, 2] = 0.0f; rm[3, 3] = 1.0f;

        tm[0, 0] = 1.0f; tm[0, 1] = 0.0f; tm[0, 2] = 0.0f; tm[0, 3] = -pe.x;
        tm[1, 0] = 0.0f; tm[1, 1] = 1.0f; tm[1, 2] = 0.0f; tm[1, 3] = -pe.y;
        tm[2, 0] = 0.0f; tm[2, 1] = 0.0f; tm[2, 2] = 1.0f; tm[2, 3] = -pe.z;
